...
from .data_utils import (createmaskdf, load_measurement,
                                        get_measurement_variable, shift_df)
...
def pandas2file(df_to_file_func, out_file):
    ...
    with SoftFileLock(out_file+'lock'):
        tic()
...
def get_contest_mask():
    raise NotImplementedError("get_contest_mask not implemented")
...
def get_first_year(data_id):
    ...
    """
    if data_id.startswith("era5-f"):
        return 1999 # quintiles data begins 20 years after original data
    if data_id.startswith("era5"):
        return 1979
...
def get_ground_truth(gt_id, mask_df=None, shift=None):
    ...
    """
    gt_file = os.path.join(cloud_storage, "data",
                           "dataframes", "gt-"+gt_id+"-14d.h5")
    ...
    fp = fs.open(gt_file)
    return load_measurement(fp, mask_df, shift)
...
def get_deadline_delta(target_horizon):
    """Returns number of days between official forecast date
    and start date of target period
    (18 for days 19-25 target, as it's 18 days away,
    25 for days 26-32 target, as it's 25 days away)

    Args:
       target_horizon: "19" or "26" indicating whether target period is
          days 19-25 or days 26-32
    """
    if target_horizon == "19":
        deadline_delta = 18
    elif target_horizon == "26":
        deadline_delta = 25
    else:
...
def get_forecast_delta(target_horizon, days_early=0):
    ...
    (deadline_delta + days_early, as we forecast early)
...
def get_measurement_lag(data_id):
    ...
    # Most of our measurements require 7 days of aggregation
    aggregation_days = 7
    # Some of our measurements are also released a certain number of days late
    days_late = 0
    if data_id.startswith("era5-"): 
        # Data arrives 4 days late, but this is offset by producing the forecast 2 days late
        # (e.g., producing the forecast on Saturday instead of Thursday)
        days_late = 2
