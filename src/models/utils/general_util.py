...
def set_dir_permissions(file_path, skip_if_exists=False, throw=False,
                        mode='777'):
    """Recursively set permissions of directory and all members of directory.

    Parameters
    ----------
    skip_if_exists : boolean
        If True, skips setting permissions if dir exists
    ...
    if not skip_if_exists or (skip_if_exists and not os.path.exists(file_path)):
        # Set permissions
        try:
            os.system(f'chmod -R {mode} {file_path}')
