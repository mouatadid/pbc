...
# %%
"""
Predicts outcomes using debiased AIFS forecasts

Example usages:
  python src/models/debiased_aifs/batch_predict.py era5-f3_tas 19 -t std_aifs_forecast

  for dates in std_aifs_forecast; do
    for var in tas mslp pr; do
      for f in f1 f2 f3 f4; do
        for horizon in 19 26; do
          src/batch/batch_python.sh --memory 10 --cores 1 --hours 1 src/models/debiased_aifs/batch_predict.py era5-${f}_${var} ${horizon} -t $dates;   
        done;
      done;
    done;
  done
...
model_name = "debiased_aifs"
submodel_name = "debiased_aifs"
...
target_lead = pd.Timedelta(days=int(horizon))
forecast_data = forecast_data.sel(lead=target_lead, drop=True).load()

tic()
printf(f'Shifting dataset by {aifs_delta} days')
forecast_data = forecast_data.assign_coords(time=forecast_data.issuance_date + pd.Timedelta(days=aifs_delta))
