...
# %%
""" 
Probabilistic Bias Correction (PBC) for AIFS dynamical model

For each gridpoint and target date, regress onto the specified ensemble members' forecasts 

Example usage:
  python src/models/pbc_aifs/batch_predict.py era5-f1_tas 19 -t std_aifs_forecast -e
...
Named args:
  ...
  --equal (-e): if True, set equal weights
  --simplex (-s): if True, constrain weights to simplex
"""
...
from models.pbc_aifs.attributes import get_submodel_name
...
if not isnotebook():
    ...
    parser.add_argument('--margin_in_days', '-m', default="None",
                        help="number of month-day combinations on either side of the target combination "
                             ...
                             "set to None to include entire year")
    parser.add_argument('--equal', '-e', default="False",
                        choices=['True', 'False'],
...
pp_name = f'proj_tuned_{forecast}pp'
