...
# %%
'''
Persistence++ with debiased AIFS AI model

Regress onto debiased AIFS forecasts, climatology, and lagged measurements

Example usages:
  python src/models/perpp_debias_aifs/batch_predict.py era5-f1_tas 19 -t std_aifs_forecast -y all -m None
  for var in tas pr mslp; do
    for f in f1 f2 f3 f4; do
      for horizon in 19 26; do
        src/batch/batch_python.sh -m 45 -c 10 -h 2 src/models/perpp_debias_aifs/batch_predict.py era5-${f}_${var} $horizon -t std_aifs_forecast
      done
    done
  done
...
measurement_variable = get_measurement_variable(gt_id)  # tas, pr, mslp, etc.
...
# The number of days between a target date and the associated AIFS model issuance date
...
aifs_col = f"aifs_{measurement_variable}"
...
pred_cols = x_cols
exclude_cols = set([clim_col, aifs_col])
...
# %%
def load_debiased_aifs_df(forecast_type="forecast", lead=aifs_delta+1):
    """Returns debiased AIFS forecast or reforecast data averaged over leads 
    [first_lead, last_lead] as a dataframe
    
    Args:
        forecast_type: "forecast" or "reforecast"
        lead: lead time in days (lead = aifs_delta+1 uses lead k to predict day k)
    """
    printf(f"Loading debiased AIFS {forecast_type} data with lead {lead}")
    tic()
    aifs_forecast_ds = load_data(os.path.join("debiased_aifs", f"debiased_aifs-{forecast_type}-{measurement_variable}"), lsmask=not gt_id.endswith("mslp"))
    toc()

    # Select single lead
    # drop=True drops the singleton lead coordinate
    return aifs_forecast_ds.sel(lead=np.timedelta64(lead, 'D'), drop=True)


# %%
#
# Load debiased AIFS reforecast data
#
reforecast_data = load_debiased_aifs_df(forecast_type="reforecast")
...
tic()
reforecast_data = reforecast_data.to_array(name=aifs_col).squeeze().to_series()
reforecast_data.dropna(inplace=True)
toc()

# %%
#
# Load debiased AIFS forecast data
#
forecast_data = load_debiased_aifs_df(forecast_type="forecast")
...
forecast_data = forecast_data.assign_coords(time=forecast_data.get_index("time").shift(freq = f"{aifs_delta}D"))
first_forecast_target = forecast_data.get_index('time').intersection(target_date_objs).min()
toc()
...
printf("Converting to Series")
tic()
forecast_data = forecast_data.to_array(name=aifs_col).squeeze().to_series()
...
# %%
# Concatenate debiased AIFS forecast and reforecast dataframes
printf(f"Combining debiased AIFS forecast and reforecast data")
tic()
aifs = pd.concat([forecast_data, reforecast_data])
del forecast_data, reforecast_data
# Drop duplicated indices, preserving the first (prioritizes forecasts over reforecasts)
aifs = aifs.loc[~aifs.index.duplicated(keep="first")]
# Sort index
aifs.sort_index(inplace=True)
toc()

# %%
# Merge debiased AIFS features with lld_data
printf(f"Merging {aifs_col} with lld_data")
tic()
lld_data = lld_data.join(aifs, how="inner")
del aifs
toc()
tic()
...
# Only forecast for targets with forecast data
sorted_targets = [date for date in sorted_targets if date >= first_forecast_target]
