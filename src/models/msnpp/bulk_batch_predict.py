...
# %%
'''
Generates predictions for each of the MSN parameter configurations
used by the tuner

Example usage: (include -p to preview commands without running batch_predict.py)
  python src/models/msnpp/bulk_batch_predict.py era5-f4_mslp 19 -t std_tune -c "src/batch/batch_python.sh -m 15 -c 1"
...
Example usages: (include -o to overwrite existing predictions)
  for var in tas pr mslp; do
    for horizon in 19 26; do
        for f in f4 f3 f2 f1; do
            python src/models/msnpp/bulk_batch_predict.py era5-${f}_${var} $horizon -t std_tune -c "src/batch/batch_python.sh -m 15 -c 1"
        done
    done
  done
  ...
  for var in tas pr mslp; do
    for horizon in 19 26; do
        for f in f4 f3 f2 f1; do
            python src/models/msnpp/bulk_batch_predict.py era5-${f}_${var} $horizon -t std_tune -mo -c "src/batch/batch_python.sh -m 10 -c 1"
        done
    done
  done
...
from models.msnpp.attributes import get_submodel_name
