...
# %%
'''
Generates predictions for each of the PBC-MSN parameter configurations

Example usage: (include -p to preview commands without running batch_predict.py)
  python src/models/pbc_msn/bulk_batch_predict.py era5-f1_tas 19 -t std_test -c "src/batch/batch_python.sh -m 20 -c 8" -p

Example usage: (exclude -p to run batch_predict.py)
  python src/models/pbc_msn/bulk_batch_predict.py era5-f1_tas 19 -t std_test -c "src/batch/batch_python.sh -m 20 -c 8" 
  python src/models/pbc_msn/bulk_batch_predict.py era5-f1_tas 19 -t std_test -c "src/batch/batch_python.sh -m 1 -c 1" 
  # (include -o to overwrite existing predictions)
  for var in tas pr mslp; do
    for f in {1..4}; do
      for horizon in 19 26; do
        python src/models/pbc_msn/bulk_batch_predict.py era5-f${f}_${var} ${horizon} -o -t std_test -c "src/batch/batch_python.sh -m 1 -c 1"
      done
    done
  done
