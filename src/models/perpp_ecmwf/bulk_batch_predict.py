...
Example usage: (include -p to preview commands without running batch_predict.py)
  python src/models/perpp_ecmwf/bulk_batch_predict.py era5-f1_mslp 19 -t std_test -c "src/batch/batch_python.sh -m 50 -c 14" -p

Example usages: (exclude -p to run batch_predict.py)
  python src/models/perpp_ecmwf/bulk_batch_predict.py era5-f1_mslp 19 -t std_test -c "src/batch/batch_python.sh -m 50 -c 14" 
  ...
  for var in mslp; do
    for f in f1 f2 f3 f4; do
      for horizon in 19 26; do
        python src/models/perpp_ecmwf/bulk_batch_predict.py era5-${f}_${var} $horizon -t std_future -c "src/batch/batch_python.sh -m 50 -c 13"
      done
    done
  done
  for var in tas pr; do
    for f in f1 f2 f3 f4; do
      for horizon in 19 26; do
        python src/models/perpp_ecmwf/bulk_batch_predict.py era5-${f}_${var} $horizon -t std_future -c "src/batch/batch_python.sh -m 35 -c 10"
      done
    done
  done 
