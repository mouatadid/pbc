...
# %%
#
# Predicts outcomes using AIFS++
#
# Example usages:
#   python src/models/aifspp/batch_predict.py era5-f1_pr 19 -t std_tune -i True -y 20 -m 35 -fl 19 -ll 19 
#
# Positional args:
...
#     AIFS predictions; if "False" does not fit intercept; (default: "False")
...
#   --days (-d): number of daily AIFS forecasts to average (default: 1)
#   --loss (-l): loss function in ["mse"] (default: "mse")
#   --first_lead (-fl): first AIFS lead to average into forecast (0-29) (default: 19)
#   --last_lead (-ll): last AIFS lead to average into forecast (0-29) (default: 19)
...
if not isnotebook():
    ...
    parser.add_argument('--days', '-d', default=1,
                       help="number of daily AIFS forecasts to average")
    parser.add_argument('--loss', '-l', default="mse", choices=["mse"],
                        help="loss function")
    parser.add_argument('--first_lead', '-fl', default=19, 
                        help="first AIFS lead to average into forecast (0-29)")
    parser.add_argument('--last_lead', '-ll', default=19, 
                        help="last AIFS lead to average into forecast (0-29)")
...
# Store the number of days between a target date and the associated AIFS model issuance date
aifs_delta = get_forecast_delta(horizon)
# This is also the amount of shift we will apply to the (re)forecast dates
base_shift_delta = timedelta(days=aifs_delta)
...
# Helper functions
#
def load_aifs_df(forecast_type="forecast"):
    ...
    """
    # Load in AIFS data
    printf(f"Loading AIFS {forecast_type} data averaged over leads [{first_lead},{last_lead}]")
    tic()
    ...
    aifs_forecast_ds = aifs_forecast_ds.sel(lead=lead_range).mean(dim='lead').load()
    toc()
...
#
print("Loading AIFS reforecast data")
...
#
print("Loading AIFS forecast data")
...
tic()
# The first target date for which an AIFS forecast exists
