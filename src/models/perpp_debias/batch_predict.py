...
# %%
#
# Persistence++ with Debiased ECMWF
#
# Regress onto Debiased ECMWF forecasts, climatology, and lagged measurements
#
# Example usage:
#   python src/models/perpp_debias/batch_predict.py era5-f4_tas 26 -t std_future -y all -m None 
#   python src/models/perpp_debias/batch_predict.py era5-F10_tas 19 -t std_test -y all -m None
#
# Positional args:
#   gt_id: era5-f1_tas, era5-f1_pr, era5-f1_mslp, era5-F90_tas, etc.
#   horizon: 19 or 26
#
# Named args:
...
def load_ecmwf_df(forecast_type="forecast", lead=ecmwf_delta+1):
    """Returns debiased ECMWF forecast or reforecast data for lead lead as a dataframe
    
    Args:
        forecast_type: "forecast" or "reforecast"
        lead: lead time in days (lead = ecmwf_delta+1 uses lead k to predict day k)
    """
    printf(f"Loading Debiased ECMWF {forecast_type} data with lead {lead}")
    tic()
    if gt_id.endswith("tas") or gt_id.endswith("pr"):
        ecmwf_forecast_ds = load_data(f"debiased_ecmwf-{forecast_type}-{measurement_variable}", lsmask=True)
    elif gt_id.endswith("mslp"):
        ecmwf_forecast_ds = load_data(f"debiased_ecmwf-{forecast_type}-{measurement_variable}", lsmask=False)
    toc()
