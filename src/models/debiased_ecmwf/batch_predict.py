...
# %%
"""
Predicts outcomes using debiased ECMWF forecasts from Zarr data
...
submodel_name = "debiased_ecmwf"

# %%
# Load debiased ECMWF forecast data
printf(f"Loading {model_name} data")
tic()
dataset_id = f"{model_name}-forecast-{gt_id.replace('era5-', '')}"
...
forecast_data = forecast_data.assign_coords(time=forecast_data.issuance_date + pd.Timedelta(days=ecmwf_delta))
