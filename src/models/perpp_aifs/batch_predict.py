...
# %%
'''
Persistence++ with AIFS AI model

Regress onto AIFS forecasts, climatology, and lagged measurements

Example usages:
  python src/models/perpp_aifs/batch_predict.py era5-f1_tas 19 -t std_aifs_forecast -y all -m None
  for var in tas pr mslp; do
    for f in f1 f2 f3 f4; do
      for horizon in 19 26; do
        src/batch/batch_python.sh -m 45 -c 10 -h 2 src/models/perpp_aifs/batch_predict.py era5-${f}_${var} $horizon -t std_aifs_forecast
      done
    done
  done
...
# %%
def load_aifs_df(forecast_type="forecast", lead=aifs_delta+1):
    """Returns AIFS forecast or reforecast data averaged over leads 
    ...
    """
    printf(f"Loading AIFS {forecast_type} data with lead {lead}")
    tic()
    aifs_forecast_ds = load_data(f"aifs-{forecast_type}-{measurement_variable}", lsmask=not gt_id.endswith("mslp"))
    toc()
...
# %%
#
# Load AIFS reforecast data
#
reforecast_data = load_aifs_df(forecast_type="reforecast")
...
#
forecast_data = load_aifs_df(forecast_type="forecast")
...
# %%
# Concatenate AIFS forecast and reforecast dataframes
...
# %%
# Merge aifs features with lld_data
