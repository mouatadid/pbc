...
# The number of days between a target date and the associated MSN model issuance date
...
msn_col = f"msn_{measurement_variable}"
...
pred_cols = x_cols
exclude_cols = set([clim_col, msn_col])
...
# %%
def load_msn_df(forecast_type="forecast", lead=msn_delta+1):
    ...
    Args:
        ...
        lead: lead time in days (lead = msn_delta+1 uses lead k to predict day k)
    """
    printf(f"Loading MSN {forecast_type} data with lead {lead}")
    tic()
    ...
    return msn_forecast_ds.sel(lead=np.timedelta64(lead, 'D'), drop=True)
...
tic()
reforecast_data = reforecast_data.to_array(name=msn_col).squeeze().to_series()
...
forecast_data = forecast_data.assign_coords(time=forecast_data.get_index("time").shift(freq = f"{msn_delta}D"))
...
printf("Converting to Series")
tic()
forecast_data = forecast_data.to_array(name=msn_col).squeeze().to_series()
...
# %%
# Concatenate MSN forecast and reforecast dataframes
printf(f"Combining forecast and reforecast data")
tic()
msn = pd.concat([forecast_data, reforecast_data])
del forecast_data, reforecast_data
# Drop duplicated indices, preserving the first (prioritizes forecasts over reforecasts)
msn = msn.loc[~msn.index.duplicated(keep="first")]
# Sort index
msn.sort_index(inplace=True)
toc()

# %%
# Merge msn features with lld_data
printf(f"Merging {msn_col} with lld_data")
tic()
lld_data = lld_data.join(msn, how="inner")
del msn
toc()
tic()
