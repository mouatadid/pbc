...
# %%
# Generate and save ECMWF model-based quintiles from reforecast data
#
...
#   4. Only calculates quintiles for leads 19 and 26
#
# Args:
...
#              If specified, saves to {model}-quintiles-{variable}.zarr
#              If not specified, processes all models and saves to ecmwf-quintiles-{variable}.zarr
#     --start_date: Optional start date for processing (YYYYMMDD)
#     --end_date: Optional end date for processing (YYYYMMDD)
#     --force: Overwrite any existing ECMWF model-based quintiles
#
# Example usage:
#   # Process dates for a specific model with optional date range filtering
#   python src/data/datasets/ecmwf-quintiles.py -wv tas --model ecmwf_cy49 -v
#   python src/data/datasets/ecmwf-quintiles.py -wv pr --model ecmwf_cy48 --start_date 20230101 --force
#   python src/data/datasets/ecmwf-quintiles.py -wv tas --model ecmwf_cy41-47 --end_date 20151224 
#   src/batch/batch_python.sh -m 50 -c 8 -h 12 src/data/datasets/ecmwf-quintiles.py -wv pr --model ecmwf_cy49 -v
#
...
parser.add_argument(
    '--force',
    action='store_true',
    help='Overwrite any existing ECMWF model-based quintiles'
)
...
# %%
def compute_ecmwf_quintiles_for_date(
    issuance_date: datetime,
    ...
    """
    Compute ECMWF model-based quintiles for a single issuance date based on SUBS-M-climate recipe.
    ...
    Returns:
        xarray.Dataset with quintile thresholds indexed by (quantile, lead, latitude, longitude),
    ...
    # over issuance_date, hindcast_date, and M dimensions 
    quantiles = [0.2, 0.4, 0.6, 0.8]

    printf(f"Computing quintiles...")
    tic()
    quintile_thresholds = pooled_data.quantile(quantiles, dim=['issuance_date', 'hindcast_date', 'M'])
    toc()

    # Create output dataset with proper variable name and coordinates
    output = xr.Dataset(
        {measurement_variable: quintile_thresholds},
        coords={
            'quantile': quantiles,
...
printf(f"Generating ECMWF quintiles for {measurement_variable}")
...
if args.model:
    ...
    dataset_name = f"{args.model}-quintiles-{measurement_variable}"
else:
    ...
    dataset_name = f"ecmwf-quintiles-{measurement_variable}"
...
quintiles_subdir = os.path.join(DATA_DIR, "ecmwf-quintiles")
...
dataset_relpath = os.path.join("ecmwf-quintiles", dataset_name)
...
for i, issuance_date in enumerate(issuance_dates):
    ...
    # Compute quintiles for this issuance date
    # Save pooled_data across iterations to avoid reloading and recomputing for overlapping base dates
    quintiles_ds, pooled_data = compute_ecmwf_quintiles_for_date(
        issuance_date=issuance_date,
    ...
    if quintiles_ds is not None:
        # Expand issuance_date from scalar coord to dimension for proper concatenation
        quintiles_ds = quintiles_ds.expand_dims('issuance_date')
...
printf(f"\nSuccessfully saved {dates_saved} new dates to ecmwf-quintiles/{dataset_name}")
